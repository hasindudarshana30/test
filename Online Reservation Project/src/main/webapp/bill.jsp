<%@page import="project.ConnectionProvider"%>
<%@page import="java.sql.*"%>
<%@include file="footer.jsp"%>
<html>
<head>
<link rel="stylesheet" href="css/bill.css">
<title>Bill</title>
</head>
<body>
	<%
String email=session.getAttribute("email").toString();
try
{
	int total=0;
	int sno=0;
	Connection con=ConnectionProvider.getCon();
	Statement st=con.createStatement();
	ResultSet rs=st.executeQuery("select sum(total) from cart where email='"+email+"' and status='bill'");
	while(rs.next())
	{
		total=rs.getInt(1);
	}
	ResultSet rs2=st.executeQuery("select *from user inner join cart where cart.email='"+email+"' and cart.status='bill'");
while(rs2.next())
{
%>
	<h3>Online Reservation System Bill</h3>
	<hr>
	<div class="left-div">
		<h3>
			Name:
			<%=rs2.getString(2) %></h3>
	</div>
	<div class="right-div-right">
		<h3>
			Email:
			<%out.println(email); %>
		</h3>
	</div>
	<div class="right-div">
		<h3>
			Contact Number:
			<%=rs2.getString(22) %></h3>
	</div>

	<div class="left-div">
		<h3>
			Check In Date:
			<%=rs2.getString(23) %></h3>
	</div>
	<div class="right-div-right">
		<h3>
			Payment Method:
			<%=rs2.getString(25) %></h3>
	</div>
	<div class="right-div">
		<h3>
			Check out Date:
			<%=rs2.getString(24) %></h3>
	</div>

	<div class="left-div">
		<h3>
			Transaction Id:
			<%=rs2.getString(26) %></h3>
	</div>
	<div class="right-div-right">
		<h3>
			City:
			<%=rs2.getString(19) %></h3>
	</div>
	<div class="right-div">
		<h3>
			Address:
			<%=rs2.getString(18) %></h3>
	</div>

	<div class="left-div">
		<h3>
			State:
			<%=rs2.getString(20) %></h3>
	</div>
	<div class="right-div-right">
		<h3>
			Country:
			<%=rs2.getString(21) %></h3>
	</div>

	<hr>
	<%break;} %>


	<br>

	<table id="customers">
		<h3>Room Details</h3>
		<tr>
			<th>S.No</th>
			<th>Room Type</th>
			<th>category</th>
			<th>Price</th>
			<th>Number_Of_Nights</th>
			<th>Sub Total</th>
		</tr>
		<%
  ResultSet rs1=st.executeQuery("select * from cart inner join product where cart.room_id=product.id and cart.email='"+email+"'and cart.status='bill'");
  while(rs1.next())
  {
	  sno=sno+1;
  %>
		<tr>
			<td>
				<%out.println(sno); %>
			</td>
			<td><%=rs1.getString(17) %></td>
			<td><%=rs1.getString(18) %></td>
			<td><%=rs1.getString(19) %></td>
			<td><%=rs1.getString(3) %></td>
			<td><%=rs1.getString(5) %></td>
		</tr>
		<tr>
			<%} %>
		
	</table>
	<h3>
		Total:
		<%out.println(total); %>
	</h3>
	<a href="continueShopping.jsp"><button class="button left-button">Continue
			Booking</button></a>
	<a onclick="window.print();"><button class="button right-button">Print</button></a>
	<br>
	<br>
	<br>
	<br>
	<%
}
catch(Exception e)
{
	System.out.println(e);
	}%>
</body>
</html>