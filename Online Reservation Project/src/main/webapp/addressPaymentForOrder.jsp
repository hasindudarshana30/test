...
%>
		...
		<tbody>
			<%
      ResultSet rs=st.executeQuery("select *from product inner join cart on product.id=cart.room_id and cart.email='"+email+"' and cart.address is NULL");
     while(rs.next())
     {
      %>
			<tr>
	...
	<form action="addressPaymentForOrderAction.jsp" method="post">
		<div class="left-div">
			...
			<input class="input-style" type="text" name="address"
				value="<%=rs2.getString(5)%>" placeholder="Enter Address" required>
		</div>

		<div class="right-div">
			<h3>Enter city</h3>
			<input class="input-style" type="text" name="city"
				value="<%=rs2.getString(10)%>" placeholder="Enter City" required>
		</div>

		<div class="left-div">
			<h3>Enter State</h3>
			<input class="input-style" type="text" name="state"
				value="<%=rs2.getString(11)%>" placeholder="Enter State" required>
		</div>

		<div class="right-div">
			<h3>Enter country</h3>
			<input class="input-style" type="text" name="country"
				value="<%=rs2.getString(12)%>" placeholder="Enter Country" required>
		</div>
		...
		<div class="right-div">
			<h3>Pay online on this oceanviewresort@pay.com</h3>
		...
		<div class="left-div">
			<h3>Mobile Number</h3>
			<input class="input-style" type="text" name="contact_number"
				value="<%=rs2.getString(6)%>" placeholder="Enter Mobile Number"
				required>
		...
		<div class="right-div">
			<h3 style="color: red">*If you enter wrong transaction id then
				your booking will we can cancel!</h3>
			<button class="button" type="submit">
				Procced to Generate Bill & Save <i
